...
const DEFAULT_IMAGE = "/images/statue.png";
...
vec2 coverUv(vec2 uv, float frameAspect, float imageAspect) {
  ...
  vec2 mapped = (uv - 0.5) * (axis * scale + (1.0 - axis)) + 0.5;
  vec2 anchor = vec2(0.42, 0.32);
  return mapped + (anchor - 0.5) * axis * (1.0 - scale);
}
`;
...
function ParticleField({
  ...
  return (
    <>
      {showImage && (
        <mesh frustumCulled={false} renderOrder={-1} material={backdropMaterial}>
...
const ParticleImage: React.FC<ParticleImageProps> = ({
  ...
  return (
    <div
      ref={root}
      className={["particle-image-root", className].filter(Boolean).join(" ")}
    ...
    >
      ...
      {children && <div className="particle-image-content">{children}</div>}
    </div>
  );
};
